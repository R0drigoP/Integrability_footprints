...
## /main
The main directory has the code used to generate random matrices (rand_matrices.jl) and compute the respective eigenvalues (eig_from_file.jl), as well as how to calculate the complex spacing ratios (CSR.jl). It also contains two notebooks, one where we compute the angular velocity and the other used to plot the data as in the main text. Important: It was not possible to import most of the eigenvalues due to memory restrictions; thus, the Plots notebook will not run successfully (only Figure 2 can be obtained since it does not need eigenvalues explicitly).
